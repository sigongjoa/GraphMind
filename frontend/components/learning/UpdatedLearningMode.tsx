...
import { conceptsApi, llmApi } from '../../api';

const UpdatedLearningMode: React.FC = () => {
  ...
  const [llmStatus, setLlmStatus] = useState<{ status: string; message?: string }>({ status: 'unknown' });
  ...
  useEffect(() => {
    const checkLlmStatus = async () => {
      try {
        ...
        setLlmStatus({ status: 'offline', message: '연결할 수 없습니다' });
      } finally {
  ...
  const handleSendMessage = async (message: string) => {
    ...
    try {
      ...
      if (llmStatus.status === 'online') {
        // LLM 서비스 사용
        response = await llmApi.explainConcept(concept.name, message);
      } else {
        // 목업 응답 사용
        response = {
...
export default UpdatedLearningMode;